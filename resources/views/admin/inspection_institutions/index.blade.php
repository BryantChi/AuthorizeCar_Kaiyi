...
@push('page_scripts')
    ...
    <script>
        $(function() {
            let scrollX_enable = "{{ count($inspectionInstitutions) > 0 ? 1 : 0 }}" == true;
            if($(window).width() > 1200) { scrollX_enable = false }
            else { scrollX_enable = "{{ count($inspectionInstitutions) > 0 ? 1 : 0 }}" == true; }

            var table = $('#inspectionInstitutions-table').DataTable({
