            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <a class="btn btn-primary float-right"
                       href="{{ route('admin.carFuelCategories.create') }}">
    ...
    <div class="content px-3">
        ...
        <div class="card">
            <div class="card-body p-0">
                @include('admin.car_fuel_categories.table')

                {{-- <div class="card-footer clearfix">
                    <div class="float-right">
                        @include('adminlte-templates::common.paginate', ['records' => $carFuelCategories])
                    </div>
                </div> --}}
            </div>
...
@push('page_scripts')
    ...
    <script>
        $(function() {
            let scrollX_enable = "{{ count($carFuelCategories) > 0 ? 1 : 0 }}" == true;
            if($(window).width() > 1200) { scrollX_enable = false }
            else { scrollX_enable = "{{ count($carFuelCategories) > 0 ? 1 : 0 }}" == true; }

            var table = $('#carFuelCategories-table').DataTable({
